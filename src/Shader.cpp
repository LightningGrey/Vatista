...
namespace Vatista {
	...
	void Shader::Compile(const char* vs_source, const char* fs_source) {
		...
		if (success == GL_FALSE) {
			...
			if (length > 0) {
				...
				VATISTA_LOG_ERROR("Shader failed to link:\n{}", log);
				delete[] log;
			}
			else {
				VATISTA_LOG_ERROR("Shader failed to link for an unknown reason!");
			}
			...
			VATISTA_LOG_TRACE("Shader has been linked");
		}
	}
	...
	GLuint Shader::__CompileShaderPart(const char* source, GLenum type) {
		...
		if (compileStatus == GL_FALSE) {
			...
			// Dump error log
			VATISTA_LOG_ERROR("Failed to compile shader part:\n{}", log);
			...
			VATISTA_LOG_TRACE("Shader part has been compiled!");
		}
